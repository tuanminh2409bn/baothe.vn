...
class _HomeScreenState extends ConsumerState<HomeScreen> {
                    ...
                    cardsAsync.when(
                      data: (cards) {
                        final List<CreditCard> filteredCards = cards.where((card) {
                          final matchesBank = selectedBank == null || 
                              card.bankName.trim().toLowerCase() == selectedBank.trim().toLowerCase();
                          final query = searchQuery.toLowerCase();
                          final matchesSearch = query.isEmpty || 
                              card.name.toLowerCase().contains(query) || 
                              card.bankName.toLowerCase().contains(query) ||
                              card.cashbackHighlight.toLowerCase().contains(query);
                          return matchesBank && matchesSearch;
                        }).toList();

                        final displayCards = filteredCards.take(displayLimit).toList();
                        final bool hasMore = filteredCards.length > displayLimit;
                        final bool isSearching = searchQuery.isNotEmpty || selectedBank != null;
                        ...
                        return Column(
                          children: [
                            Row(
                              children: [
                                Text('Tìm thấy ', style: AppStyles.bodyMedium.copyWith(fontSize: 18)),
                                Text('${filteredCards.length} thẻ ', style: AppStyles.h2.copyWith(color: AppColors.accentOrange, fontSize: 20)),
                                if (selectedBank != null) Text('tại $selectedBank', style: AppStyles.bodyMedium.copyWith(fontSize: 18)),
                                const Spacer(),
                                TextButton(
                                  onPressed: () {
                                  ...
                                  child: const Text('Xóa tất cả bộ lọc', style: TextStyle(color: Colors.red, fontSize: 16)),
                                ),
                              ],
                            ),
                            ...
                            if (filteredCards.isEmpty) 
                              Padding(
                                ...
                                child: Text('Không tìm thấy thẻ phù hợp. Hãy thử từ khóa khác!', style: TextStyle(color: AppColors.textSecondary, fontSize: 16)),
                              )
                            else ...[
          ...
          child: AnimatedHover(
            ...
            child: InkWell(
              onTap: () {
                ...
                ref.read(searchQueryProvider.notifier).state = cat;
              },
  ...
  Widget _buildFullBanksSection(WidgetRef ref, String? currentSelected) {
    ...
    return Column(
      children: [
        ...
        ], height: h1Height, horizontalSpacing: commonSpacing, containerWidth: h1ContainerWidth, currentSelected: currentSelected),
        const SizedBox(height: 20),
        _buildLogoRow(ref, [
        ...
        ], height: commonHeight, horizontalSpacing: commonSpacing, containerWidth: commonContainerWidth, currentSelected: currentSelected),
  ...
  Widget _buildLogoRow(WidgetRef ref, List<Map<String, String>> items, {double height = 50, double horizontalSpacing = 10, double containerWidth = 140, String? currentSelected}) {
    return Wrap(
      ...
      children: items.map((item) {
        ...
        return AnimatedHover(scale: 1.1, child: InkWell(onTap: () => ref.read(selectedBankProvider.notifier).state = item['name'], borderRadius: BorderRadius.circular(8), child: Container(width: containerWidth, height: height + 15, alignment: Alignment.center, decoration: BoxDecoration(color: Colors.transparent, border: Border.all(color: isSelected ? AppColors.accentOrange : Colors.transparent, width: 1.5), borderRadius: BorderRadius.circular(8)), child: Image.asset('assets/logo/${item['file']}', height: height, fit: BoxFit.contain, errorBuilder: (context, error, stackTrace) => Text(item['name']!, style: const TextStyle(fontSize: 10, color: Colors.grey), textAlign: TextAlign.center)))));
      }).toList(),
    );
  }
