...
def cleanup_ocb():
    ...
    print("🧹 Bắt đầu dọn dẹp dữ liệu OCB...")

    # 1. Xóa trong Firestore
    cards_ref = db.collection("cards")
    query = cards_ref.where("bankName", "==", "OCB").stream()
    count_db = 0
    for doc in query:
        print(f"  - Đang xóa document: {doc.id}")
        cards_ref.document(doc.id).delete()
        count_db += 1
    print(f"✅ Đã xóa {count_db} thẻ OCB khỏi Firestore.")

    # 2. Xóa ảnh trong Storage
    blobs = bucket.list_blobs(prefix="card_images/ocb_")
    count_img = 0
    for blob in blobs:
        print(f"  - Đang xóa ảnh: {blob.name}")
        blob.delete()
        count_img += 1
    print(f"✅ Đã xóa {count_img} ảnh OCB khỏi Storage.")
    print("\n✨ HOÀN THÀNH DỌN DẸP!")
