...
def cleanup_lpbank():
    ...
    print("🧹 Bắt đầu dọn dẹp dữ liệu LPBank...")

    # 1. Xóa trong Firestore
    cards_ref = db.collection("cards")
    query = cards_ref.where("bankName", "==", "LPBank").stream()
    ...
    print(f"✅ Đã xóa {count_db} thẻ LPBank khỏi Firestore.")

    # 2. Xóa ảnh trong Storage
    blobs = bucket.list_blobs(prefix="card_images/lpbank_")
    count_img = 0
    for blob in blobs:
    ...
    print(f"✅ Đã xóa {count_img} ảnh LPBank khỏi Storage.")
    print("\n✨ HOÀN THÀNH DỌN DẸP!")
